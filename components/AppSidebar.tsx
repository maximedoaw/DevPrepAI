...
function NotificationsButton({
  ...
  return (
    <Link
      ...
      className="block w-full text-left p-3 rounded-xl hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-colors hover:shadow-md relative"
    >
...
function SidebarContent({ children }: { children: React.ReactNode }) {
  ...
  if (!userDataLoading && userRole) {
    ...
    if (userRole === "CAREER_CHANGER") {
      sidebarOptions = [
        ...
        {
          id: "community",
          title: "Communauté Reconversion",
          description: "Connectez-vous avec d'autres personnes en reconversion",
          icon: Users,
          color: "text-green-600 dark:text-green-400",
          bgColor: "bg-green-500 dark:bg-green-600",
          action: () => {
  ...
  return (
    <>
      ...
      {!shouldHideSidebar && (
        <div
          className={`fixed inset-y-0 left-0 z-50 bg-white dark:bg-slate-900 border-r border-emerald-100 dark:border-emerald-900/50 transition-all duration-300 ease-in-out flex flex-col ${sidebarOpen ? "w-80 translate-x-0" : "-translate-x-full md:translate-x-0 md:w-20"
            }`}
        >
          {/* Header Sidebar */}
                ...
                return (
                  <div key={option.id} className="relative group">
                    <Link
                      ...
                      className={`block w-full text-left p-3 rounded-xl transition-all duration-200 group ${isActive
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-lg ${option.bgColor} flex-shrink-0 transition-transform duration-200 group-hover:scale-110 ${option.isAdmin ? "ring-2 ring-red-200 dark:ring-red-800" : ""
                            }`}
                        >
                        ...
                        {sidebarOpen && (
                          <div className="flex-1 min-w-0 z-10 relative">
                            <div className="flex items-center gap-2 mb-1">
                              <span
                                className={`font-semibold truncate bg-gradient-to-r bg-clip-text text-transparent ${option.isAdmin
                        ...
                        {sidebarOpen && (
                          <ChevronRight
                            className={`h-4 w-4 text-emerald-400 transition-transform duration-200 flex-shrink-0 ${hoveredOption === option.id ? "translate-x-1" : ""
                              }`}
                          />
                        )}
                      </div>
              ...
              <div className="mt-6 px-3">
                ...
                <div className="space-y-1">
                  <Link
                    ...
                    className="block w-full text-left p-3 rounded-xl hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-colors hover:shadow-md"
                  >
                    <div className="flex items-center gap-3">
            ...
            {sidebarOpen && user ? (
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800">
                  <Avatar className="w-10 h-10 flex-shrink-0 shadow-md border-2 border-emerald-200 dark:border-emerald-800">
                    <AvatarImage src={userData?.imageUrl || user.picture || undefined} alt={`${user.given_name} ${user.family_name}`} />
              ...
              <div className="flex justify-center">
                <Avatar className="w-10 h-10 shadow-md border-2 border-emerald-200 dark:border-emerald-800">
                  <AvatarImage src={userData?.imageUrl || user?.picture || undefined} alt={user ? `${user.given_name} ${user.family_name}` : "User"} />
      ...
      {/* Main content */}
      <div
        className={`min-h-screen bg-white dark:bg-slate-950 transition-all duration-300 ${sidebarOpen ? "md:ml-80" : "md:ml-20"
          }`}
      >
        {/* Page content */}
